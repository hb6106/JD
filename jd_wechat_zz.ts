...
import {get, randomNumString, requireConfig, wait} from './TS_USER_AGENTS'
    ...
    for (let [index, value] of cookiesArr.entries()) {
        ...
        for (let t of res.data.taskDetailResList) {
            if (t.times === 0) {
                ...
                res = await get(`https://api.m.jd.com/client.action?functionId=doInteractTask&body=${encodeURIComponent(JSON.stringify({
                    "taskId": t.taskId,
                    "taskItem": taskItem,
                    "actionType": 0,
                    "taskToken": t.taskToken,
                    "mpVersion": "3.4.0"
                }))}&appid=wh5&loginWQBiz=interact&g_ty=ls&g_tk=${randomNumString(9)}`, headers)
