                ...
                if (t.ext) {
                    if (t.assignmentName === '每日签到') {
                        if (t.ext.sign1.status === 1) {
                            ...
                            res = await api(`functionId=doInteractiveAssignment&body=${JSON.stringify({
                                ...
                                "ext": {},
                                "extParam": {
                                    "businessData": {"random": log.match(/"random":"(\d+)"/)[1]},
                                    "signStr": log.match(/"log":"(.*)"/)[1],
                                    "sceneid": "XMFhPageh5"
                                }
                    ...
                    for (let proInfo of t.ext.productsInfo ?? []) {
                        if (proInfo.status === 1) {
                            ...
                            log = await getLog()
                            res = await api(`functionId=doInteractiveAssignment&body=${encodeURIComponent(JSON.stringify({
                                "encryptProjectId": sign,
                                "encryptAssignmentId": t.encryptAssignmentId,
                                "sourceCode": "acexinpin0823",
                                "itemId": proInfo.itemId,
                                "actionType": 0,
                                "completionFlag": "",
                                "ext": {},
                                "extParam": {
