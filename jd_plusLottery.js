...
async function main() {
    ...
    let queryInteractiveInfo = await doApi("queryInteractiveInfo", {
        ...
        if (vo.userVerificationInfo.userQulification) {
            if ([1].includes(vo.assignmentType)) {
                ...
                if (vo.ext[vo.ext.extraType][0].status !== 2) {
                    let doInteractiveAssignment = await doApi("doInteractiveAssignment", {
                        "encryptProjectId": "U13AR7JM6UNr2okmamEig7TD4Ef",
                        "encryptAssignmentId": vo.encryptAssignmentId,
                        "itemId": vo.ext[vo.ext.extraType][0].itemId,
                        "sourceCode": "aceaceqingzhan"
                    })
    ...
    let queryInteractiveRewardInfo = await doApi("queryInteractiveRewardInfo", {
        "encryptProjectId": "U13AR7JM6UNr2okmamEig7TD4Ef",
        "ext": {"detailEncryptAssignmentIds": [], "needExchangeRestScore": 1, "detailTypeFlag": "1"},
        "sourceCode": "aceaceqingzhan"
    })
