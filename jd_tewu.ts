...
import USER_AGENT, {getshareCodeHW, o2s, post, requireConfig, wait} from './TS_USER_AGENTS'
...
let cookie: string = '', UserName: string = '', res: any = '', message: string = '', shareCodes: ShareCode[] = [],
    shareCodesSelf: ShareCode[] = [], shareCodesHW: any = [], black: string[] = []
        ...
        for (let t of res.data.result.taskList) {
            if (t.completionCnt !== t.assignmentTimesLimit) {
                ...
                if (t.ext?.shoppingActivity || t.ext?.followShop) {
                    ...
                    res = await api('superBrandDoTask', {
                        "source": "card",
                    ...
                    for (let sign of t.ext.sign2) {
                        if (sign.status === 0 && [10, 18].includes(new Date().getHours())) {
                            res = await api('superBrandDoTask', {
                                ...
                                "encryptAssignmentId": t.encryptAssignmentId,
                                "assignmentType": t.assignmentType,
                                "itemId": t.ext.currentSectionItemId,
                                "actionType": 0
                            })
                            console.log(res.data?.bizMsg)
                            await wait(2000)
    ...
    for (let [index, value] of cookiesArr.entries()) {
        ...
        for (let code of shareCodes) {
            ...
            res = await api('superBrandDoTask', {
                ...
                "encryptProjectId": code.encryptProjectId,
                "encryptAssignmentId": code.encryptAssignmentId,
                "assignmentType": 2,
                "itemId": code.itemId,
                "actionType": 0
            })
