...
function pg_channel_page_data() {
    return new Promise((resolve) => {
        $.get(taskUrl("pg_channel_page_data", {
            "v": "13.2",
            "paramData": {"token": "2b11cbdd-b8bb-4a98-ad17-c9d6bc23ee92"},
            "argMap": {"raffleConfigId": "12"}
        }), async (err, resp, data) => {
            try {
                if (err) {
...
function pg_interact_interface_invoke(floorToken, raffleActKey) {
    return new Promise((resolve) => {
        $.get(taskUrl("pg_interact_interface_invoke", {
            "v": "13.2",
            "floorToken": floorToken,
            "dataSourceCode": "lottery",
            "argMap": {"raffleActKey": raffleActKey, "pitIndex": 1}
        }), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
